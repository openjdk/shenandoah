...
bool ShenandoahGeneration::prepare_regions_and_collection_set(bool concurrent) {
...

  {
    ...
    size_t old_regions_dedicated_to_young_evac = 0;
...
#undef KELVIN_PROMOTION_BUDGETING
    ...
    if (heap->mode()->is_generational()) {
...
#endif
      if ((old_regions_dedicated_to_young_evac > 0) && (immediate_garbage_regions > 0)) {
        if (old_regions_dedicated_to_young_evac > immediate_garbage_regions) {
          old_regions_dedicated_to_young_evac -= immediate_garbage_regions;
        } else {
          old_regions_dedicated_to_young_evac = 0;
        }
      }
      ...
      //
      //  1. old_gen->available - old_evacuation_commitment - old_regions_dedicated_to_young_evac * region_size_bytes
      ...
      size_t promotion_reserve = (old_generation->available() - old_evacuation_committed -
                                  old_regions_dedicated_to_young_evac * region_size_bytes);
#ifdef KELVIN_PROMOTION_BUDGETING
      ...
      //
      //   1. old_gen->available() -
      //        (promotion_reserve + old_evacuation_commitment + old_regions_dedicated_to_young_evac * region_size_bytes)
      ...
      // Otherwise, as described above, the reason regions_available_to_loan is less than net_available_old_regions is because
      // the available memory is scattered between many partially used regions.  

      size_t young_evacuation_reserve = heap->get_young_evac_reserve();
      size_t old_regions_loaned_for_young_evac;
      if (young_evacuation_reserve > young_generation->available()) {
        size_t short_fall = young_evacuation_reserve - young_generation->available();
        assert (regions_available_to_loan * region_size_bytes >= short_fall,
                "Young evacuation shortfall cannot exceed old-gen loan potential");
        old_regions_loaned_for_young_evac = (short_fall + region_size_bytes - 1) / region_size_bytes;
        regions_available_to_loan -= old_regions_loaned_for_young_evac;
      } else {
        old_regions_loaned_for_young_evac = 0;
