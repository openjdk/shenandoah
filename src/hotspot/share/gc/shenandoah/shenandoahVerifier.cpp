 ...
 public:
  ...
  static void validate_usage(const bool adjust_for_padding,
    ...
    if (adjust_for_padding && (generation->is_young() || generation->is_global())) {
      size_t pad = heap->collection_set_parameters()->get_pad_for_promote_in_place();
      generation_used += pad;
    }
...
void ShenandoahVerifier::verify_at_safepoint(const char* label,
  ...
  // Heap size checks
  {
    ...
    if (_heap->mode()->is_generational() && (sizeness == _verify_size_adjusted_for_padding)) {
      ...
      heap_used = _heap->used() + _heap->collection_set_parameters()->get_pad_for_promote_in_place();
